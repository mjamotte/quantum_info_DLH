...
\begin{frame}{Mesure et simulation}
  \begin{columns}[T,onlytextwidth]
    \column{0.55\textwidth}
    \begin{itemize}
      \item Ajouter un bit classique et mesurer: \texttt{qc.measure(0, 0)}.
      \item Le résultat de mesure est stocké dans le registre classique.
      \item La mesure projette l'état quantique sur \(\ket{0}\) ou \(\ket{1}\) selon les probabilités.
    \end{itemize}
    \column{0.45\textwidth}
    \centering
    \includegraphics[width=0.9\linewidth]{figures/measure_circuit.png}
    \\[-0.3em]\footnotesize Circuit avec mesure après $H$
  \end{columns}
\end{frame}
