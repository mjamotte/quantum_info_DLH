...
\begin{frame}{Stabilité de l'atome}
  \begin{itemize}[<+->]
    ...
    \begin{figure}
      ...
      \hfill
      \only<3->{\includegraphics[width=0.2\linewidth]{../figures/intro/bohr_model.png}}
    \end{figure}
...
\begin{frame}{Différences fondamentales entre ordinateurs classique et quantique}
  ...
  \begin{tabular}{l|c|c}
    ...
    & Discret & Continu \\
     ~& ~ & ~\\
    Description mathématique & Booléens (0/1) & Vecteurs/Matrices \\
    ~& ~ & ~\\
...
\begin{frame}{Utilité malgré cette limitation}
  \begin{itemize}
    \item {\bf Algorithme de Grover:} Accélération quadratique de la résolution d'un problème difficile dont il est facile de reconnaître une solution. Applications: chercher un élément dans une base de données non-structurée, inverser une fonction de hachage, ... \\~\\
    \item {\bf Algorithme de Shor:} Accélération exponentielle de la factorisation de nombres (casse le chiffrement RSA). Se base sur un algorithme quantique accélérant le calcul de la {\bf transformée de Fourier}, une technique mathématique très utile dans de nombreux domaines. \\~\\
    \item {\bf Algorithmes hybrides d'optimisation (QAOA, VQE,...):} Principales applications en chimie, avantage quantique encore débattu.
  \end{itemize}
\end{frame}
...
\begin{frame}{Ce que nous proposons}
  ...
  \begin{itemize}
    ...
    \item Qubit: représentations bra-ket et vectorielle
    \item Portes logiques quantiques et repésentation matricielle
    \item Pratique sur ordinateur avec Qiskit
    \item Mesurer l'état d'un qubit + Chiffrement quantique de clé
    \item Systèmes à plusieurs qubits + Téléportation quantique
    \item Algorithme de Grover
    \item Plateformes physiques permettant de réaliser des ordinateurs quantiques
    \item Possibilité de discuter d'autres aspects pour approfondir...
  \end{itemize}
\end{frame}
